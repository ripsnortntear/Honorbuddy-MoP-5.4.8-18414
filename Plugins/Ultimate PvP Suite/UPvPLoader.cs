...
namespace UltimatePvPSuiteLoader
{
    ...
    {
        private static string FolderLocation = Path.Combine(Utilities.AssemblyDirectory, "Plugins\\" + DirectoryLocator());
            ...
            try
            {
                ...
                AppDomain.CurrentDomain.AssemblyResolve += delegate(object sender, ResolveEventArgs e)
                {
                    try
                    {
                        ...
                        if (requestedName.Name == "Tripper.Tools")
                        {
                            //Logging.Write("Loading " + Utilities.AssemblyDirectory + @"\GreyMagic.dll");
                ...
                if (File.Exists(Utilities.AssemblyDirectory + @"\BuddyPlugins\BuddyPluginsAutoUpdater.exe"))
                {
                    string[] args =
                    {
                        "Guest", "Guest", "UltimatePvPSuite",
                        String.Format("\"" + FolderLocation + "\"")
                    };
                    var p = new Process
                    {
                        StartInfo =
                        {
                            FileName =
                                Path.Combine(Utilities.AssemblyDirectory + @"\BuddyPlugins\",
                                    "BuddyPluginsAutoUpdater.exe"),
        ...
        private static string DirectoryLocator()
        {
            try
            {
                foreach (
