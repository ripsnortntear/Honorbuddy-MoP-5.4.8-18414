...
namespace MageHelper
{
    public partial class MageHelper
    {
        ...
        public void Invisibility()
        {
            ...
            if (UseInvis)
            {
                ...
                if (AddList.Count >= InvisAdds && Me.HealthPercent <= InvisHP && SpellManager.CanCast("Invisibility"))
                {
                    ...
                    Me.ClearTarget();
                    AddList = null;
                    BotPoi.Clear("MageHelper exiting combat to save a life.");
                }
            }
        }
        ...
        {
            if (UseBlinkKill && BotPoi.Current.Type == PoiType.Kill && BotPoi.Current.Location.Distance(Me.Location) > 25 && !Me.Mounted)
            {
                //Blink towards Poi's we need to kill, if over 25 yards away and not mounted
                if (SpellManager.CanCast("Blink"))
                {
                    BlinkOnlyWithPerfection();
                    BotPoi.Current.AsObject.ToUnit().Face(); StyxWoW.SleepForLagDuration(); SpellManager.Cast("Blink"); Log("Blinked towards: ", BotPoi.Current.Name); WoWMovement.ClickToMove(BotPoi.Current.Location); 
                }

            }
            if (Me.CurrentTarget == null && UseBlinkLoot && Styx.Helpers.CharacterSettings.Instance.LootMobs.Equals(true))
            {
        ...
        private void PortalsRule()
        {
            ...
            if (!IsStarted && (CurrentPOI.Type == PoiType.Mail || CurrentPOI.Type == PoiType.Repair || CurrentPOI.Type == PoiType.Sell) && !BotManager.Current.Name.ContainsAny(BadBotBases))
            {
            ...
            {
                if (InventoryManager.HaveItemsToMail && Styx.Helpers.CharacterSettings.Instance.MailRecipient != null)
                {
                    ObjectManager.Update();
                    Log("Mailing Items");
        ...
        {
            if (AlwaysIceB && Me.Combat && !Me.IsDead && !Me.Mounted)
            {
                if (!Me.HasAura("Ice Barrier") && (SpellManager.CanCast(11426) || SpellManager.CanCast("Ice Barrier")))
                {
                    SpellManager.Cast("Ice Barrier", Me);
                    Log("Casted Ice Barrier on myself!");
                }
            }
        }
