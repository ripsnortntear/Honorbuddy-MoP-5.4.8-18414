...
namespace Eclipse.EclipsePlugins.Controllers
{
    public class EclipseDBProfile
    {
        ...
        {
            XDocument newDoc = new XDocument(new XElement("DungeonBuddyProfile")) { Declaration = new XDeclaration("1.0", "utf-16", "yes") };
            newDoc.Element("DungeonBuddyProfile").Add(new XElement("Name") { Value = Name.Replace(".xml", "") });
            newDoc.Element("DungeonBuddyProfile").Add(new XElement("DungeonId") { Value = DungeonId });
            return true;
        }
        #endregion
    }
