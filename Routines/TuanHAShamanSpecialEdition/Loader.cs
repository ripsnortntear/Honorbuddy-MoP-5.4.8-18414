...
namespace Loader
{
    public class Loader : CombatRoutine
    {
        ...
        public Loader()
        {
            string settingsDirectory = Path.Combine(Utilities.AssemblyDirectory, "Routines\\TuanHAShamanSpecialEdition");

            string path = settingsDirectory + @"\TuanHA_Combat_Routine.dll";

            if (!File.Exists(path))
            {
                MessageBox.Show("TuanHAShamanSpecialEdition is not installed correctly! Ensure files look like;" + 
                            Environment.NewLine + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/Files/<.doc .jpg>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/Preset/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/User Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/ChangeLog.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/Loader.cs" + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/Please Read - Installation Guide.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAShamanSpecialEdition/TuanHA_Combat_Routine.dll");
                return;
            }
            ...
            foreach (FileInfo file in DInfo.GetFiles())
            {
                ...
                {
                    removed = true;
                    Logging.Write("Removing " + file.Name + " from TuanHAShamanSpecialEdition directory");
                    file.Delete();
                }
            }
            //if (removed)
            //{
            //    MessageBox.Show("Removed unnecessary files from TuanHAShamanSpecialEdition directory, if TuanHAShamanSpecialEdition fails to work please restart honorbuddy.");
            //}
            ...
            {
                Logging.Write(Colors.DarkRed, "An error occured while loading TuanHAShamanSpecialEdition!");
