...
namespace Loader
{
    public class InsanityLoader : CombatRoutine
    {
        private CombatRoutine CC;


        private readonly String[] Keep = new[] { "InsanityLoader.cs", "Insanity.dll", "Settings", "Changelog.txt" };

        public InsanityLoader()
        {
            string settingsDirectory = Path.Combine(Utilities.AssemblyDirectory, "Routines\\Insanity");

            string path = settingsDirectory + @"\Insanity.dll";

            if (!File.Exists(path))
            {
                MessageBox.Show("Insanity is not installed correctly! Ensure files look like;" + 
                            Environment.NewLine + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/Insanity/Insanity.dll" + Environment.NewLine +
                            "<Honorbuddy>/Routines/Insanity/InsanityLoader.cs" + Environment.NewLine +
                            "<Honorbuddy>/Routines/Insanity/Settings/<XML Files>");
                return;
            }
            bool removed = false;
            ...
            foreach (FileInfo file in DInfo.GetFiles())
            {
                ...
                {
                    removed = true;
                    Logging.Write("Removing " + file.Name + " from Insanity directory");
                    file.Delete();
                }
            }
            if (removed)
            {
                MessageBox.Show("Removed unnecessary files from Insanity directory, if Insanity fails to work please restart honorbuddy.");
            }
            ...
            {
                Logging.Write(Colors.DarkRed, "An error occured while loading Insanity!");
