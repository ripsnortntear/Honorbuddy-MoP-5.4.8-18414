 ...
 * $ID$
 * $Revision: 201 $
 * $URL: https://subversion.assembla.com/svn/oracle/trunk/Oracle/Core/CombatLog/CombatLogHandler.cs $
 * $LastChangedBy: wulf_ $
 * $ChangesMade$
 */
...
namespace Oracle.Core.CombatLog
{
    [UsedImplicitly]
    internal class CombatLogHandler
    {
        ...
        private static void HandleCombatLogEvent(object sender, LuaEventArgs args)
        {
            ...
            {
                Logger.Output(args.Args[1] + ", Index: " + index);
                Logger.Output("{0}", ex);
            }
        }
        ...
        public static void Remove(string combatLogEventName)
        {
            ...
            if (EventHandlers.TryGetValue(combatLogEventName, out handlers))
            {
                ...
                Logger.Output(" Removed combat filter:  {0}", combatLogEventName);
            }
        }
    }
