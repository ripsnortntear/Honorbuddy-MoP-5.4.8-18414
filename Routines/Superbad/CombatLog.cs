...
namespace Superbad
{
    internal class CombatLogEventArgs : LuaEventArgs
    {
        ...
        {
            get { return Args[7].ToString().Length > 1 ? ulong.Parse(Args[7].ToString().Replace("0x", string.Empty), NumberStyles.HexNumber) : 0; }
        }
        ...
        {
            if (o != null && o.ToString() != "" && o.ToString().Length > 1)
            {
                ...
                {
                    Logging.Write("error parsing Guid '{0}'", o);
                }
            }
