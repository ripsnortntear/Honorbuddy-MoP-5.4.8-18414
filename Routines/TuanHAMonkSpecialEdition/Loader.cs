...
namespace Loader
{
    public class Loader : CombatRoutine
    {
        ...
        public Loader()
        {
            string settingsDirectory = Path.Combine(Utilities.AssemblyDirectory, "Routines\\TuanHAMonkSpecialEdition");

            string path = settingsDirectory + @"\TuanHA_Combat_Routine.dll";

            if (!File.Exists(path))
            {
                MessageBox.Show("TuanHAMonkSpecialEdition is not installed correctly! Ensure files look like;" + 
                            Environment.NewLine + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/Files/<.doc .jpg>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/Preset/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/User Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/ChangeLog.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/Loader.cs" + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/Please Read - Installation Guide.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAMonkSpecialEdition/TuanHA_Combat_Routine.dll");
                return;
            }
            ...
            foreach (FileInfo file in DInfo.GetFiles())
            {
                ...
                {
                    removed = true;
                    Logging.Write("Removing " + file.Name + " from TuanHAMonkSpecialEdition directory");
                    file.Delete();
                }
            }
            //if (removed)
            //{
            //    MessageBox.Show("Removed unnecessary files from TuanHAMonkSpecialEdition directory, if TuanHAMonkSpecialEdition fails to work please restart honorbuddy.");
            //}
            ...
            {
                Logging.Write(Colors.DarkRed, "An error occured while loading TuanHAMonkSpecialEdition!");
