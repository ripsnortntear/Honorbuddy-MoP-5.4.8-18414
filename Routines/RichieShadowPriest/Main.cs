...
namespace RichieShadowPriestPvP {
    ...
    public partial class Main : CombatRoutine {
        ...
        public override WoWClass Class {
            get {
                if (Me.Class == WoWClass.Priest && StyxWoW.Me.Specialization == WoWSpec.PriestShadow)
        ...
        void BotEvents_OnBotStarted(EventArgs args) {
            ...
            if (SPSettings.Instance.RightClickMovementOff && TreeRoot.Current.Name != "BGBuddy")
            ...
            Spells.OnReloaded = () => {
                ...
                Spells.AddAdditional(SpellIDs.MindFlay, "Mind Flay");
                Spells.AddAdditional(SpellIDs.Halo_Shadow, "Halo");
                Spells.AddAdditional(SpellIDs.Cascade_Shadow, "Cascade");
                Spells.AddAdditional(SpellIDs.DivineStar_Shadow, "Divine Star");
            };
            ...
            try
            {
                ...
                if (objects.Count > 0)
                {
                    ...
                    SPSettings.Instance.BuffDispelASAPHS.Clear();
                    foreach (XmlNode o in objects)
                        SPSettings.Instance.BuffDispelASAPHS.Add(Convert.ToInt32(o.Attributes["id"].Value));
                }

                Logging.WriteDiagnostic("New values of BuffDispelASAPHS:");
                foreach (int i in SPSettings.Instance.BuffDispelASAPHS)
                ...
                if (objects.Count > 0)
                {
                    ...
                    SPSettings.DebuffCCPurifyASAPHS.Clear();
                    foreach (XmlNode o in objects)
                        SPSettings.DebuffCCPurifyASAPHS.Add(Convert.ToInt32(o.Attributes["id"].Value));
                }

                Logging.WriteDiagnostic("New values of DebuffCCPurifyASAPHS:");
                foreach (int i in SPSettings.DebuffCCPurifyASAPHS)
                    Logging.WriteDiagnostic("" + i);
            }
            catch (Exception e)
            {
        ...
        private void OnGlyphsReloaded()
        {
            ...
            UnitManager.HasSWDGlyph = GlyphManager.Has(Glyphs.GlyphOfShadowWordDeath);
            CanUseFadeAsDefCD = GlyphManager.Has(Glyphs.GlyphOfFade);
        }
        ...
        public override void Pulse()
        {
            ...
            if (LastScan.AddMilliseconds(SPSettings.Instance.SearchInterval) <= DateTime.Now)
            {
                ...
                using (StyxWoW.Memory.AcquireFrame())
                {
                    ...
                    using (var perf = PerfLogger.GetHelper("RefreshUnits"))
                        ...
                        UnitManager.Refresh();
#if MEASURE_PERFORMANCE
                    using (var perf = PerfLogger.GetHelper("GetMultidotTarget"))
#endif
                        UnitManager.GetMultidotTarget();
                }
            }
...
/*            DateTime started = DateTime.Now;
            ...
            */
            ...
            Logging.Write("LastDispel.AddSeconds(SPSettings.Instance.DispelDelay) <= DateTime.Now: " + (LastDispel.AddSeconds(SPSettings.Instance.DispelDelay) <= DateTime.Now));// &&
            Logging.Write("HealTarget.HealthPercent >= SPSettings.Instance.DispelAboveHp: " + (HealTarget.HealthPercent >= SPSettings.Instance.DispelAboveHp));// &&
            ...
            */
            ...
            //Logging.Write("hold: " + (SPSettings.Instance.DontCancelDominateMind && Me.IsCasting && Me.CastingSpellId == 605));
