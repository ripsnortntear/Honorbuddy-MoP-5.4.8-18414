...
namespace Loader
{
    ...
    /// </summary>
    [UsedImplicitly]
    public class GladiatorSuiteDeathKnight : CombatRoutine
    {
        #region Tidy : Properties

        private static readonly string SDir = Path.Combine(Utilities.AssemblyDirectory,
            @"Routines\GladiatorSuite\DeathKnight");

        private readonly string _dllPath = SDir + @"\GSDeathKnight.dll";
        ...
        public GladiatorSuiteDeathKnight()
        {
            //Check Installation
        ...
        private bool ValidateInstallation()
        {
            try
            {
                ...
                MessageBox.Show(
                    "Dll not found",
                    "GladiatorSuite is not installed correctly", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            catch (Exception ex)
            {
