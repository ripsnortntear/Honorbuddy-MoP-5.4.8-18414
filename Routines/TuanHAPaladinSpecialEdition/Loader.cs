...
namespace Loader
{
    public class Loader : CombatRoutine
    {
        ...
        public Loader()
        {
            string settingsDirectory = Path.Combine(Utilities.AssemblyDirectory, "Routines\\TuanHAPaladinSpecialEdition");

            string path = settingsDirectory + @"\TuanHA_Combat_Routine.dll";

            if (!File.Exists(path))
            {
                MessageBox.Show("TuanHAPaladinSpecialEdition is not installed correctly! Ensure files look like;" + 
                            Environment.NewLine + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/Files/<.doc .jpg>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/Preset/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/User Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/ChangeLog.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/Loader.cs" + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/Please Read - Installation Guide.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHAPaladinSpecialEdition/TuanHA_Combat_Routine.dll");
                return;
            }
            ...
            foreach (FileInfo file in DInfo.GetFiles())
            {
                ...
                {
                    removed = true;
                    Logging.Write("Removing " + file.Name + " from TuanHAPaladinSpecialEdition directory");
                    file.Delete();
                }
            }
            //if (removed)
            //{
            //    MessageBox.Show("Removed unnecessary files from TuanHAPaladinSpecialEdition directory, if TuanHAPaladinSpecialEdition fails to work please restart honorbuddy.");
            //}
            ...
            {
                Logging.Write(Colors.DarkRed, "An error occured while loading TuanHAPaladinSpecialEdition!");
