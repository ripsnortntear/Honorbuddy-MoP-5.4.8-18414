...
namespace Loader
{
    public class Loader : CombatRoutine
    {
        ...
        public Loader()
        {
            string settingsDirectory = Path.Combine(Utilities.AssemblyDirectory, "Routines\\TuanHARogueSpecialEdition");

            string path = settingsDirectory + @"\TuanHA_Combat_Routine.dll";

            if (!File.Exists(path))
            {
                MessageBox.Show("TuanHARogueSpecialEdition is not installed correctly! Ensure files look like;" + 
                            Environment.NewLine + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/Files/<.doc .jpg>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/Preset/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/User Settings/<XML Files>" +
							Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/ChangeLog.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/Loader.cs" + 
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/Please Read - Installation Guide.txt" +
                            Environment.NewLine +
                            "<Honorbuddy>/Routines/TuanHARogueSpecialEdition/TuanHA_Combat_Routine.dll");
                return;
            }
            ...
            foreach (FileInfo file in DInfo.GetFiles())
            {
                ...
                {
                    removed = true;
                    Logging.Write("Removing " + file.Name + " from TuanHARogueSpecialEdition directory");
                    file.Delete();
                }
            }
            //if (removed)
            //{
            //    MessageBox.Show("Removed unnecessary files from TuanHARogueSpecialEdition directory, if TuanHARogueSpecialEdition fails to work please restart honorbuddy.");
            //}
            ...
            {
                Logging.Write(Colors.DarkRed, "An error occured while loading TuanHARogueSpecialEdition!");
